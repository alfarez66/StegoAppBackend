class DeflateCompression {
    ...
    huffmanEncode(data, codes) {
        return data.map(item => {
            if (item.char !== undefined) {
                ...
                return codes[`(${item.distance},${item.length})`];
            }
        }).join('');
    }
        ...
        for (let bit of binaryString) {
            ...
            if (invertedCodes.hasOwnProperty(code)) {
                ...
                if (symbol.startsWith('(')) {
                    const match = symbol.slice(1, -1).split(',');
    ...
    parseDistanceLengthPair(pair) {
        const match = pair.slice(1, -1).split(',');
        return { distance: parseInt(match[0]), length: parseInt(match[1]) };
    }
    ...
    lz77Decompress(encodedItems) {
        ...
        for (const item of encodedItems) {
            if (item.char !== undefined) {
                ...
                let pattern = '';
                // Handle case where length exceeds distance
                for (let i = 0; i < item.length; i++) {
                    pattern += output[start + i];
                }
                output += pattern;
            }
        }
    ...
    deflate(input) {
        ...
        const frequencies = lz77Output.reduce((acc, item) => {
            const symbol = item.char !== undefined ? item.char : `(${item.distance},${item.length})`;
